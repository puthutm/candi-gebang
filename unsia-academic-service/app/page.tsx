...
export default function AcademicDashboard() {
  ...
  // Fetch students from academic backend API
  const fetchStudents = async () => {
    try {
      const hostname = typeof window !== 'undefined' ? window.location.hostname : 'localhost';
      const response = await fetch(`http://${hostname}:3006/api/v1/students`);
      const result = await response.json();
      if (response.ok && result.data && result.data.length > 0) {
        const mapped: Student[] = result.data.map((item: any) => ({
          id: item.id,
          nim: item.nim || '2601012003',
          name: item.biodata ? `${item.biodata.firstName} ${item.biodata.lastName}` : 'Mahasiswa Baru',
          program: 'PJJ Informatika',
          gpa: 3.82,
          krsStatus: 'SUBMITTED',
          advisor: 'Dr. Eri Prasetyo',
        ...
        setStudents(mapped);
        console.log('Academic active: loaded data from academic_db.');
      }
    } catch (err) {
      console.warn('Academic operating in degraded fallback mode (service offline).');
    } finally {
      setIsLoading(false);
    }
  };
                      ...
                      >
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="h-5 w-5 rounded-md border flex items-center justify-center transition-all"
                               style={{
                                 backgroundColor: isChecked ? '#4f46e5' : '#0f172a',
                                 borderColor: isChecked ? '#4f46e5' : '#1e293b',
                                 color: '#ffffff'
                               }}>
                            {isChecked && '✓'}
                          </div>
