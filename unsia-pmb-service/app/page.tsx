...
export default function PmbDashboard() {
  ...
  // Fetch applicants from PMB backend API
  const fetchApplicants = async () => {
    try {
      const hostname = typeof window !== 'undefined' ? window.location.hostname : 'localhost';
      const response = await fetch(`http://${hostname}:3004/api/v1/applicants`);
      const result = await response.json();
      if (response.ok && result.data && result.data.length > 0) {
        const mapped: Applicant[] = result.data.map((item: any) => ({
          id: item.id,
          name: item.biodata ? `${item.biodata.firstName} ${item.biodata.lastName}` : 'Unregistered Candidate',
          email: item.email,
          phone: item.phone,
          path: item.registrationPath || 'Regular',
          status: item.status || 'SUBMITTED',
          payment: item.status === 'HANDED_OVER' || item.status === 'LOA_ISSUED' ? 'PAID' : 'UNPAID',
          examScore: item.examScore || 80,
          docs: {
            ktp: item.documents?.find((d: any) => d.documentType === 'KTP')?.status || 'PENDING',
            ijazah: item.documents?.find((d: any) => d.documentType === 'IJAZAH')?.status || 'PENDING',
            photo: item.documents?.find((d: any) => d.documentType === 'PHOTO')?.status || 'PENDING',
          }
        }));
        setApplicants(mapped);
        console.log('PMB active: loaded data from pmb_db.');
      }
    } catch (err) {
      console.warn('PMB operating in degraded fallback mode (service offline).');
    } finally {
      setIsLoading(false);
    }
  };
                      ...
                      {examQuestions[currentQuestion].options.map((opt, optIdx) => {
                        ...
                        return (
                          <button
                            ...
                            style={{
                              backgroundColor: isSelected ? 'rgba(79, 70, 229, 0.1)' : 'rgba(2, 6, 23, 0.4)',
                          ...
                          >
                            <span className="h-6 w-6 rounded-lg flex items-center justify-center font-bold font-mono text-[10px]"
                                  style={{
                                    backgroundColor: isSelected ? '#4f46e5' : '#0f172a',
